DATASET="mega"
INPUT_HADOOP="/input/gutenberg-mega"
INPUT_SPARK="hdfs:///input/gutenberg-mega"
LOCAL_INPUT="/var/backups/hadoop/backup_before_reinstall/AllDatasets/Mega"
...
hdfs dfs -du -s -h "$INPUT_HADOOP" 2>/dev/null | tee -a "$ANALYSIS_DIR/dataset_info.txt" || true
...
for path in \
  "$OUT_BASE/$DATASET/hadoop-base-r1" \
  "$OUT_BASE/$DATASET/hadoop-base-r2" \
  "$OUT_BASE/$DATASET/hadoop-base-r4" \
  "$OUT_BASE/$DATASET/hadoop-base-r8" \
  "$OUT_BASE/$DATASET/hadoop-base-r16" \
  "$OUT_BASE/$DATASET/hadoop-base-r24" \
  "$OUT_BASE/$DATASET/hadoop-inmapper-r1" \
  "$OUT_BASE/$DATASET/hadoop-inmapper-r2" \
  "$OUT_BASE/$DATASET/hadoop-inmapper-r4" \
  "$OUT_BASE/$DATASET/hadoop-inmapper-r8" \
  "$OUT_BASE/$DATASET/hadoop-inmapper-r16" \
  "$OUT_BASE/$DATASET/hadoop-inmapper-r24" \
  "$OUT_BASE/$DATASET/spark-optimized-p1" \
  "$OUT_BASE/$DATASET/spark-optimized-p2" \
  "$OUT_BASE/$DATASET/spark-optimized-p4" \
  "$OUT_BASE/$DATASET/spark-optimized-p8" \
  "$OUT_BASE/$DATASET/spark-optimized-p16" \
  "$OUT_BASE/$DATASET/spark-optimized-p24"
do
...
cat > "$SUMMARY_DIR/sequential_summary.txt" <<EOF2
...
Includes Hadoop Base, Hadoop InMapper, Spark with safe memory, and Sequential Python.
